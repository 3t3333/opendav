...
impl OpenDavApp {
            ...
            egui::SidePanel::left("details_tab_strip")
                ...
                .show(ctx, |ui| {
                    ...
                    let tab_configs = [
                        ...
                        (crate::GraphsSidebarTab::Notes, "SIMGIT"),
                    ];
    ...
    fn draw_notes_sidebar_content(&mut self, ui: &mut egui::Ui, is_dark: bool) {
        ...
        if self.simgit_import_receiver.is_some() {
            ui.label(
                egui::RichText::new(
                    "Repository import is in progress. Notes will unlock when it completes.",
                )
                .color(theme.warning),
            );
            return;
        ...
        if let Some(note_id) = self.active_simgit_note_id.clone() {
                ...
                .find(|n| n.id == note_id)
            {
                ...
                let back_clicked =
                    crate::simgit::ui::note_view_widget::draw_dedicated_note_view_widget(
                        ui, note_ctx,
                    );
                if back_clicked {
                    ...
                    egui::Frame::NONE
                        ...
                        .show(ui, |ui| {
                            ui.horizontal(|ui| {
                                ...
                                .stroke(egui::Stroke::new(1.0, note.color.display_color(is_dark)))
                                .corner_radius(4.0)
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if let Some(delta) = note.context.section_delta {
                                    let delta_str =
                                        crate::simgit::repository::format_section_delta(delta);
                                    let delta_clr = crate::simgit::repository::section_delta_color(
                                        delta, is_dark,
                                    );
                                    ui.label(
